...
+ (instancetype)sharedSingleton {
    static BookNoticeView *_sharedSingleton = nil;
...
- (void)getRequest{
    ...
    WEAK_SELF;
    [AFHTTP requestInfoagreement333333success:^(NSDictionary *responseObject){
...
- (void)showMenu{
    ...
    _title.text = NSBundleLocalizedString(@"预定须知");
    [_queren setTitle:NSBundleLocalizedString(@"立即订票") forState:UIControlStateNormal];
...
- (void)lijidignpiao:(UIButton *)button{
    if(self.confirmBookNoticeViewBloak){
        self.confirmBookNoticeViewBloak();
    }
}
...
- (UIButton *)quxiao{
    if(!_quxiao){
        ...
        _quxiao.titleLabel.font = [UIFont systemFontOfSize:16];
